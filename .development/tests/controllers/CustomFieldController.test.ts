...
import CustomFieldController from '../../src/controllers/CustomFieldController';
import { createTestOrganization, createTestUser, testDb } from '../setup';

const app = express();
app.use(express.json());
app.use('/customfield', CustomFieldController);
...
describe('CustomFieldController', () => {
  ...
  describe('HTTP Endpoints', () => {
    it('should handle GET requests', async () => {
      const response = await request(app).get('/customfield');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle not found scenarios', async () => {
      const response = await request(app)
        .get('/customfield/non-existent-id');
