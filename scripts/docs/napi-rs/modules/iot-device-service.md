...
The IotDevice Service provides:

- High-performance iot device management and monitoring
...
```bash
npm install @turbo-asset/iot-device-service
```
...
### IotDeviceService Class

#### Constructor
```typescript
new IotDeviceService()
```

Creates a new instance of the IotDevice Service.
...
**Example:**
```javascript
const health = iotdeviceservice.healthCheck();
...
**Example:**
```javascript
const info = iotdeviceservice.getServiceInfo();
console.log(info);
// {
...
**Example:**
```javascript
import { init } from '@turbo-asset/iot-device-service';
const result = init();
console.log(result); // "IotDeviceService initialized successfully"
```
...
```javascript
import { IotDeviceService } from '@turbo-asset/iot-device-service';
...
const serviceInfo = iotdeviceservice.getServiceInfo();
...
```javascript
try {
  await iotdeviceservice.initialize(config);
  console.log('IotDevice service initialized successfully');
} catch (error) {
...
await Promise.all([
  iotdeviceservice.initialize(iotdeviceserviceConfig),
...
```javascript
// test/iot-device-service.test.js
import { IotDeviceService } from '@turbo-asset/iot-device-service';
...
describe('IotDeviceService', () => {
  ...
  test('should return service info', () => {
    ...
    expect(info.name).toBe('iot-device-service');
...
```javascript
// test/integration/iot-device-service.integration.test.js
import { IotDeviceService } from '@turbo-asset/iot-device-service';

describe('IotDeviceService Integration', () => {
  let service;
...
cd turbo-asset/packages/iot-device-service
npm install
npm run build
```
