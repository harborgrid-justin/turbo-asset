...
The Chargeback Service provides:

- High-performance cost allocation and chargeback management
...
```bash
npm install @turbo-asset/chargeback-service
```
...
Creates a new instance of the Chargeback Service.
...
await chargebackservice.initialize({
...
**Example:**
```javascript
const health = chargebackservice.healthCheck();
...
**Example:**
```javascript
const info = chargebackservice.getServiceInfo();
console.log(info);
// {
//   name: "chargeback-service",
//   version: "1.0.0", 
//   description: "Cost allocation and chargeback management"
// }
```
...
**Example:**
```javascript
import { init } from '@turbo-asset/chargeback-service';
const result = init();
console.log(result); // "ChargebackService initialized successfully"
```
...
```javascript
import { ChargebackService } from '@turbo-asset/chargeback-service';
...
const serviceInfo = chargebackservice.getServiceInfo();
...
```javascript
try {
  await chargebackservice.initialize(config);
  console.log('Chargeback service initialized successfully');
} catch (error) {
...
await Promise.all([
  chargebackservice.initialize(chargebackserviceConfig),
...
```javascript
// test/chargeback-service.test.js
import { ChargebackService } from '@turbo-asset/chargeback-service';
...
describe('ChargebackService', () => {
  ...
  test('should return service info', () => {
    ...
    expect(info.name).toBe('chargeback-service');
...
```javascript
// test/integration/chargeback-service.integration.test.js
import { ChargebackService } from '@turbo-asset/chargeback-service';

describe('ChargebackService Integration', () => {
  let service;
...
cd turbo-asset/packages/chargeback-service
npm install
npm run build
```
