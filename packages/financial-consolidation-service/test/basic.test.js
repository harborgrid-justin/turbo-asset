...
const { FinancialConsolidation, init } = require('../index');
...
test('service creation', async (t) => {
  const service = new FinancialConsolidation();
  assert.ok(service instanceof FinancialConsolidation);
});
...
test('service health check', async (t) => {
  ...
  assert.strictEqual(health.service, 'financial-consolidation-service');
});
...
test('service info', async (t) => {
  ...
  assert.strictEqual(info.name, 'financial-consolidation-service');
