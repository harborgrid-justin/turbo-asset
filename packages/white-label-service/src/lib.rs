...
#[napi]
pub struct WhiteLabelServiceService {
  // Service implementation
}

#[napi]
impl WhiteLabelServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "white-label-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "White label customization and branding".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("WhiteLabelServiceService initialized successfully".to_string())
}