...
/// Maintenance management and work order processing service
#[napi]
...
impl MaintenanceService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "maintenance-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Maintenance management and work order processing service".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("MaintenanceService initialized successfully".to_string())
}