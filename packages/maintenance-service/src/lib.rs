...
#[napi]
pub struct MaintenanceServiceService {
  // Service implementation
}

#[napi]
impl MaintenanceServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "maintenance-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Maintenance operations and scheduling".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("MaintenanceServiceService initialized successfully".to_string())
}