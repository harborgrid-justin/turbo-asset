...
#[napi]
pub struct DocumentServiceService {
  // Service implementation
}

#[napi]
impl DocumentServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "document-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Document management and content operations".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("DocumentServiceService initialized successfully".to_string())
}