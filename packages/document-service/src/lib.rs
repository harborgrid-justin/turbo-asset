...
/// Document management service with version control and metadata processing
#[napi]
...
impl DocumentService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "document-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Document management service with version control and metadata processing".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("DocumentService initialized successfully".to_string())
}