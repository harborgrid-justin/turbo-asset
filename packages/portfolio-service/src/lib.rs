...
#[napi]
pub struct PortfolioServiceService {
  // Service implementation
}

#[napi]
impl PortfolioServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "portfolio-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Portfolio analytics and performance tracking".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("PortfolioServiceService initialized successfully".to_string())
}