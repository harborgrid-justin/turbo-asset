...
#[napi]
pub struct MoveManagementServiceService {
  // Service implementation
}

#[napi]
impl MoveManagementServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "move-management-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Space move operations and coordination".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("MoveManagementServiceService initialized successfully".to_string())
}