...
impl WorkflowEngineService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "workflow-engine-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Configurable workflow automation engine".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("WorkflowEngineService initialized successfully".to_string())
}