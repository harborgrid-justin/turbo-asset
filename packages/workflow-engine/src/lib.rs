...
/// Configurable workflow engine with approval chains
#[napi]
...
impl WorkflowEngine {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "workflow-engine".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Configurable workflow engine with approval chains".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("WorkflowEngine initialized successfully".to_string())
}