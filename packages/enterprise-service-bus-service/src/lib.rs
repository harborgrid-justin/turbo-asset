...
#[napi]
pub struct EnterpriseServiceBusServiceService {
  // Service implementation
}

#[napi]
impl EnterpriseServiceBusServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "enterprise-service-bus-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Enterprise service bus and messaging".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("EnterpriseServiceBusServiceService initialized successfully".to_string())
}