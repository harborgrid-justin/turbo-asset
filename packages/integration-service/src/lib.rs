...
/// Enterprise integration and API management service
#[napi]
...
impl IntegrationService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "integration-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Enterprise integration and API management service".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("IntegrationService initialized successfully".to_string())
}