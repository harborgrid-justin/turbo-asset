...
#[napi]
pub struct CriticalDateServiceService {
  // Service implementation
}

#[napi]
impl CriticalDateServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "critical-date-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Critical date tracking and milestone management".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("CriticalDateServiceService initialized successfully".to_string())
}