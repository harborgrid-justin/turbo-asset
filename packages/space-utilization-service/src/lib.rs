...
#[napi]
pub struct SpaceUtilizationServiceService {
  // Service implementation
}

#[napi]
impl SpaceUtilizationServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "space-utilization-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Space utilization analytics and optimization".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("SpaceUtilizationServiceService initialized successfully".to_string())
}