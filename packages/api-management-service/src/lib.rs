...
#[napi]
pub struct ApiManagementServiceService {
  // Service implementation
}

#[napi]
impl ApiManagementServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "api-management-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "API lifecycle management and governance".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("ApiManagementServiceService initialized successfully".to_string())
}