...
const { ApiManagement, init } = require('../index');
...
test('service creation', async (t) => {
  const service = new ApiManagement();
  assert.ok(service instanceof ApiManagement);
});
...
test('service health check', async (t) => {
  ...
  assert.strictEqual(health.service, 'api-management-service');
});
...
test('service info', async (t) => {
  ...
  assert.strictEqual(info.name, 'api-management-service');
