...
#[napi]
pub struct PreventiveMaintenanceServiceService {
  // Service implementation
}

#[napi]
impl PreventiveMaintenanceServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "preventive-maintenance-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Preventive maintenance scheduling and management".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("PreventiveMaintenanceServiceService initialized successfully".to_string())
}