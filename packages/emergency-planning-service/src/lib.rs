...
#[napi]
pub struct EmergencyPlanningServiceService {
  // Service implementation
}

#[napi]
impl EmergencyPlanningServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "emergency-planning-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Emergency planning and response management".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("EmergencyPlanningServiceService initialized successfully".to_string())
}