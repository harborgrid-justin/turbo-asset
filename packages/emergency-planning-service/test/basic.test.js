...
const { EmergencyPlanning, init } = require('../index');
...
test('service creation', async (t) => {
  const service = new EmergencyPlanning();
  assert.ok(service instanceof EmergencyPlanning);
});
...
test('service health check', async (t) => {
  ...
  assert.strictEqual(health.service, 'emergency-planning-service');
});
...
test('service info', async (t) => {
  ...
  assert.strictEqual(info.name, 'emergency-planning-service');
