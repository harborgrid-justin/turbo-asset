...
#[napi]
pub struct BulkDataServiceService {
  // Service implementation
}

#[napi]
impl BulkDataServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "bulk-data-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Bulk data processing and batch operations".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("BulkDataServiceService initialized successfully".to_string())
}