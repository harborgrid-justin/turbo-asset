...
#[napi]
pub struct IotDeviceServiceService {
  // Service implementation
}

#[napi]
impl IotDeviceServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "iot-device-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "IoT device management and monitoring".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("IotDeviceServiceService initialized successfully".to_string())
}