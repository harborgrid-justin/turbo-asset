...
/// IoT device management and sensor data processing service
#[napi]
...
impl IotDeviceService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "iot-device-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "IoT device management and sensor data processing service".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("IotDeviceService initialized successfully".to_string())
}