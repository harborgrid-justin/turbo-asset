...
const { SpaceStandards, init } = require('../index');
...
test('service creation', async (t) => {
  const service = new SpaceStandards();
  assert.ok(service instanceof SpaceStandards);
});
...
test('service health check', async (t) => {
  ...
  assert.strictEqual(health.service, 'space-standards-service');
});
...
test('service info', async (t) => {
  ...
  assert.strictEqual(info.name, 'space-standards-service');
