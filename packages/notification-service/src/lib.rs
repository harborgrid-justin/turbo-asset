...
#[napi]
pub struct NotificationServiceService {
  // Service implementation
}

#[napi]
impl NotificationServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "notification-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "High-performance notification and messaging service with multi-channel delivery".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("NotificationServiceService initialized successfully".to_string())
}