...
#[napi]
pub struct VendorBrokerServiceService {
  // Service implementation
}

#[napi]
impl VendorBrokerServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "vendor-broker-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Vendor management and broker integration".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("VendorBrokerServiceService initialized successfully".to_string())
}