...
#[napi]
pub struct WorkOrderServiceService {
  // Service implementation
}

#[napi]
impl WorkOrderServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "work-order-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Work order management and tracking".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("WorkOrderServiceService initialized successfully".to_string())
}