...
/// Work order management and technician dispatch service
#[napi]
...
impl WorkOrderService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "work-order-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Work order management and technician dispatch service".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("WorkOrderService initialized successfully".to_string())
}