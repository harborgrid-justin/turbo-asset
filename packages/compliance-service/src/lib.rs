...
/// Regulatory compliance and audit trail service
#[napi]
...
impl ComplianceService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "compliance-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Regulatory compliance and audit trail service".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("ComplianceService initialized successfully".to_string())
}