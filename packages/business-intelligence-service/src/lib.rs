...
/// Business intelligence and analytics service with ML capabilities
#[napi]
...
impl BusinessIntelligenceService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "business-intelligence-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Business intelligence and analytics service with ML capabilities".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("BusinessIntelligenceService initialized successfully".to_string())
}