...
#[napi]
pub struct BudgetForecastServiceService {
  // Service implementation
}

#[napi]
impl BudgetForecastServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "budget-forecast-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Budget planning and forecasting".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("BudgetForecastServiceService initialized successfully".to_string())
}