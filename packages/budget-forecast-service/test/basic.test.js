...
const { BudgetForecast, init } = require('../index');
...
test('service creation', async (t) => {
  const service = new BudgetForecast();
  assert.ok(service instanceof BudgetForecast);
});
...
test('service health check', async (t) => {
  ...
  assert.strictEqual(health.service, 'budget-forecast-service');
});
...
test('service info', async (t) => {
  ...
  assert.strictEqual(info.name, 'budget-forecast-service');
