...
#[napi]
pub struct ContractLifecycleServiceService {
  // Service implementation
}

#[napi]
impl ContractLifecycleServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "contract-lifecycle-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Contract lifecycle management and automation".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("ContractLifecycleServiceService initialized successfully".to_string())
}