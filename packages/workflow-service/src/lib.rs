...
#[napi]
pub struct WorkflowServiceService {
  // Service implementation
}

#[napi]
impl WorkflowServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "workflow-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Advanced workflow operations and automation".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("WorkflowServiceService initialized successfully".to_string())
}