...
const { DataGovernance, init } = require('../index');
...
test('service creation', async (t) => {
  const service = new DataGovernance();
  assert.ok(service instanceof DataGovernance);
});
...
test('service health check', async (t) => {
  ...
  assert.strictEqual(health.service, 'data-governance-service');
});
...
test('service info', async (t) => {
  ...
  assert.strictEqual(info.name, 'data-governance-service');
