...
#[napi]
pub struct InventoryServiceService {
  // Service implementation
}

#[napi]
impl InventoryServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "inventory-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Inventory management and tracking operations".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("InventoryServiceService initialized successfully".to_string())
}