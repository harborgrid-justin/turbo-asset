...
/// Dynamic custom field management and validation service
#[napi]
...
impl CustomFieldService {
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "custom-field-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Dynamic custom field management and validation service".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("CustomFieldService initialized successfully".to_string())
}