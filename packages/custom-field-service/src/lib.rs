...
#[napi]
pub struct CustomFieldServiceService {
  // Service implementation
}

#[napi]
impl CustomFieldServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "custom-field-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Dynamic custom field management and configuration".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("CustomFieldServiceService initialized successfully".to_string())
}