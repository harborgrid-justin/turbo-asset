...
#[napi]
pub struct ChargebackServiceService {
  // Service implementation
}

#[napi]
impl ChargebackServiceService {
  #[napi(constructor)]
  pub fn new() -> Self {
    Self {}
  }
  ...
  pub fn get_service_info(&self) -> Result<HashMap<String, String>> {
    ...
    info.insert("name".to_string(), "chargeback-service-service".to_string());
    info.insert("version".to_string(), "1.0.0".to_string());
    info.insert("description".to_string(), "Cost allocation and chargeback management".to_string());
    Ok(info)
  }
}
...
pub fn init() -> Result<String> {
  Ok("ChargebackServiceService initialized successfully".to_string())
}