...
const { TechnicianMobile, init } = require('../index');
...
test('service creation', async (t) => {
  const service = new TechnicianMobile();
  assert.ok(service instanceof TechnicianMobile);
});
...
test('service health check', async (t) => {
  ...
  assert.strictEqual(health.service, 'technician-mobile-service');
});
...
test('service info', async (t) => {
  ...
  assert.strictEqual(info.name, 'technician-mobile-service');
